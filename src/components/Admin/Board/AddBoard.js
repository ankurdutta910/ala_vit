...
const AddBoard = () => {
  ...
  const [globalError, setGlobalError] = useState(""); // ✅ global error
  ...
  // ✅ Handle file with size validation
  const handleFileChange = (e) => {
    ...
    if (selectedFile) {
      if (selectedFile.size > 1024 * 1024) {
        ...
        setFile(null);
        setPreview(""); // remove preview
        return;
      }
      setGlobalError(""); // clear error if valid
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let uploadData = {};
      if (file) {
        uploadData = await uploadFile(file); // if fails, catch will handle
      }

      // ✅ Step 2: Insert data only if upload succeeded
  ...
  // ✅ Generate options like 2023-24, 2024-25, 2025-26
  ...
  useEffect(() => {
    if (!data.year) {
      const defaultYear = `${currentYear}-${(currentYear + 1)
        .toString()
        .slice(-2)}`;
      setData((prev) => ({ ...prev, year: defaultYear }));
    }
          ...
          <div className="content">
            <div>
              ...
              <div className="row mt-3">
                ...
                <div className="col-lg-8 col-my-2">
                  {globalError && (
                    <Message negative>
                    ...
                    <Form onSubmit={handleSubmit}>
                      ...
                      <div className="row mb-3">
                        ...
                        <div className="col-sm-6 mt-2">
                          <Form.Select
                            error={
                              errors.year ? { content: errors.year } : null
                            }
                            label="Year"
                            name="year"
